...
describe("Phase 8 Core skill and approval APIs", () => {
  it("runs a governed social approval workflow on an org-scoped Nova credential", async () => {
    ...
    const skillCompleted = events.find((event) => event.event_type === "nova.skill.completed");
    expect(skillCompleted?.output_hash).toBe(artifactHash);
    const finalLedger = await pool.query<{ response_hash: string | null }>(
      `SELECT response_hash FROM audit.ledger_entries
       WHERE organization_id = $1 AND execution_id = $2 AND entry_type = 'final'`,
      [org.organization_id, executionId]
    );
    expect(finalLedger.rows[0].response_hash).toBe(artifactHash);
    expect(artifactHash).toBe(sha256(artifact));
  ...
  it("final response_hash prefers nova.skill.completed artifact over raw model output", async () => {
    const org = await createOrg(app, platformToken, "Nova Artifact Org");
    const admin = await login(app, org.email, org.password);
    const member = await app.inject({
      method: "POST",
      url: "/v1/users",
    ...
    const executed = await app.inject({
      ...
      payload: {
        ...
        risk_tier: "medium",
        data_classification: "internal",
        content: "  draft with padding  \n",
        prefer_local: true,
      },
    });
    const modelHash = executed.json().output_hash as string;
    const artifactHash = sha256(`released:${executed.json().text as string}`);
    expect(artifactHash).not.toBe(modelHash);
    const requested = await app.inject({
      method: "POST",
      url: `/internal/v1/executions/${executionId}/approval/request`,
      headers: novaAuth,
    ...
    expect(finalized.statusCode).toBe(200);
    const events = await listExecutionEvents(pool, org.organization_id, executionId);
    const completed = events.find((event) => event.event_type === "nova.skill.completed");
    expect(completed?.output_hash).toBe(artifactHash);
    const finalLedger = await pool.query<{ response_hash: string | null }>(
      `SELECT response_hash FROM audit.ledger_entries
       WHERE organization_id = $1 AND execution_id = $2 AND entry_type = 'final'`,
      [org.organization_id, executionId]
    ...
    expect(finalLedger.rows[0].response_hash).toBe(artifactHash);
    expect(finalLedger.rows[0].response_hash).not.toBe(modelHash);
  });
